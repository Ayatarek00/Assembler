class Assembler(object):
    ...
    def __second_pass(self) -> None:
        ...
        the tables self._mri_table, self.rri_table and self._ioi_table. It should
        ...
        for i in range(len(self.__asm)):
           if (self.__asm[i][0]=='org'):
              lc = int(self.__asm[i][1],16)
               ...
               return
           elif len (self.__asm[i])==3 and self.__asm[i][1]=='dec':
                self.__bin[self.__format2bin(str(lc),'dec',12)] = self.__format2bin(str(self.__asm[i][2]),'dec',16)
                lc +=1
           elif len (self.__asm[i])==3 and self.__asm[i][1]=='hex':
           ...
           if instruction in self.__mri_table.keys():
                    ...
                    lc+=1
                        ...
                        self.__bin[self.__format2bin(str(lc),'dec',12
                        )] = str(self.__ioi_table[instruction])
                        lc+=1
        pass